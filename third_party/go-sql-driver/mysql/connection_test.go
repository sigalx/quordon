...
func TestConnectionDeadlineCapsConfiguredIOTimeouts(t *testing.T) {
	network := &deadlineRecordingConn{}
	mc := &mysqlConn{
		netConn: network,
		cfg: &Config{
	...
	if err := SetConnectionDeadline(mc, operationDeadline); err != nil {
		t.Fatalf("SetConnectionDeadline() error = %v", err)
	}

	_, _ = mc.readWithTimeout(make([]byte, 1))
	_, _ = mc.writeWithTimeout([]byte{1})
	if !network.readDeadline.Equal(operationDeadline) {
		t.Fatalf("read deadline = %v, want %v", network.readDeadline, operationDeadline)
	}
	if !network.writeDeadline.Equal(operationDeadline) {
		t.Fatalf("write deadline = %v, want %v", network.writeDeadline, operationDeadline)
	}

	if err := SetConnectionDeadline(mc, time.Time{}); err != nil {
		t.Fatalf("clear SetConnectionDeadline() error = %v", err)
	}
	if !network.deadline.IsZero() {
		t.Fatalf("cleared deadline = %v, want zero", network.deadline)
	}
}

func TestAbortConnectionClosesSocketAndMarksConnectionBad(t *testing.T) {
	network := &deadlineRecordingConn{}
	mc := &mysqlConn{
		netConn: network,
		rawConn: network,
	...
	if err := AbortConnection(mc); err != nil {
		t.Fatalf("AbortConnection() error = %v", err)
	}
	if !network.closed {
		t.Fatal("AbortConnection() did not close the physical socket")
	}
	if err := mc.error(); !errors.Is(err, driver.ErrBadConn) {
		t.Fatalf("connection error = %v, want driver.ErrBadConn", err)
	}
	if err := AbortConnection(struct{}{}); err == nil {
		t.Fatal("AbortConnection() accepted an unsupported driver connection")
	}
}

func TestOperationDeadlineSurvivesPacketErrorClassification(t *testing.T) {
	newConnection := func() *mysqlConn {
		return &mysqlConn{
			buf:               newBuffer(),
			netConn:           deadlineTimeoutConn{},
			cfg:               NewConfig(),
			closech:           make(chan struct{}),
			maxAllowedPacket:  maxPacketSize,
	...
	if _, err := newConnection().readPacket(); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("readPacket() error = %v, want operation deadline", err)
	}
	if err := newConnection().writeCommandPacketStr(comQuery, "COMMIT"); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("writeCommandPacketStr() error = %v, want operation deadline", err)
	}
}

type deadlineTimeoutConn struct{ net.Conn }

func (deadlineTimeoutConn) Read([]byte) (int, error)         { return 0, deadlineTimeoutError{} }
func (deadlineTimeoutConn) Write([]byte) (int, error)        { return 0, deadlineTimeoutError{} }
func (deadlineTimeoutConn) SetDeadline(time.Time) error      { return nil }
func (deadlineTimeoutConn) SetReadDeadline(time.Time) error  { return nil }
func (deadlineTimeoutConn) SetWriteDeadline(time.Time) error { return nil }

type deadlineTimeoutError struct{}

func (deadlineTimeoutError) Error() string   { return "operation timed out" }
