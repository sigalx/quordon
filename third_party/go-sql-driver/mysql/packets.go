...
func (mc *mysqlConn) readPacket() ([]byte, error) {
	...
	for {
		...
		data, err := readNext(4)
		if err != nil {
			deadlineErr := mc.operationDeadlineError(err)
			mc.close()
		...
		// Reject an oversized logical packet before readNext asks the buffer to
		// allocate space for its body. prevData contains earlier 16 MiB packet
		// fragments, if any.
		if mc.maxReadPacketSize > 0 &&
			(len(prevData) > mc.maxReadPacketSize || pktLen > mc.maxReadPacketSize-len(prevData)) {
			mc.close()
			return nil, ErrReadPktTooLarge
		}
