...
func (mc *mysqlConn) readWithTimeout(b []byte) (int, error) {
	deadline := mc.ioDeadline(mc.cfg.ReadTimeout)
	if !deadline.IsZero() {
...
func (mc *mysqlConn) writeWithTimeout(b []byte) (int, error) {
	deadline := mc.ioDeadline(mc.cfg.WriteTimeout)
	if !deadline.IsZero() {
...
func (mc *mysqlConn) ioDeadline(timeout time.Duration) time.Time {
	deadline := mc.operationDeadline
	if timeout <= 0 {
		return deadline
	}
	configured := time.Now().Add(timeout)
...
func (mc *mysqlConn) operationDeadlineError(err error) error {
	if err == nil || mc.operationDeadline.IsZero() || time.Now().Before(mc.operationDeadline) {
		return nil
	}
...
// SetConnectionDeadline applies an absolute operation deadline to the
// connection exposed by database/sql.Conn.Raw. It also updates the socket
// immediately so transaction Commit and Rollback, which have no context in
// database/sql/driver, cannot outlive that deadline. Configured read/write
// timeouts remain effective only when they expire earlier.
func SetConnectionDeadline(driverConn any, deadline time.Time) error {
	mc, ok := driverConn.(*mysqlConn)
	if !ok {
		return errors.New("mysql: unsupported driver connection")
	}
	if mc.netConn == nil || mc.closed.Load() {
		return ErrInvalidConn
	}
	mc.operationDeadline = deadline
	return mc.netConn.SetDeadline(deadline)
}

// AbortConnection immediately closes the physical connection exposed by
// database/sql.Conn.Raw. It is intended for callers that deliberately stop
// consuming a result set and therefore must not let Rows.Close drain the
// remaining packets synchronously. The callback passed to Conn.Raw should
// return nil after calling this function; Rows.Close will subsequently observe
// driver.ErrBadConn and release the database/sql row lock without reading from
// the socket.
func AbortConnection(driverConn any) error {
	mc, ok := driverConn.(*mysqlConn)
	if !ok {
		return errors.New("mysql: unsupported driver connection")
	}
	mc.cancel(driver.ErrBadConn)
	return nil
}
...
func (mc *mysqlConn) QueryContext(ctx context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	resetReadPacketSize := mc.applyReadPacketSize(ctx)
	...
	rows.resetReadPacketSize = resetReadPacketSize
	return rows, err
}
...
func (stmt *mysqlStmt) QueryContext(ctx context.Context, args []driver.NamedValue) (driver.Rows, error) {
	...
	resetReadPacketSize := stmt.mc.applyReadPacketSize(ctx)
