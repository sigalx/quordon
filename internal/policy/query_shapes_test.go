...
func TestQueryShapeDiscoveryBuildsBoundedImmutableAuthorization(t *testing.T) {
	...
	*cfg.Profiles["analytics"].Query.AggregateShapes[0].PublicDescription = "mutated after snapshot"
	...
	if got, want := authorized.ShapeSetHash(), "3cd5fe71171fcf7872ce90d96f1fca9201049222a3128010306490eb3fba231f"; got != want {
...
func TestQueryShapeDiscoveryRequiresExplicitV2ForTimeBuckets(t *testing.T) {
	cfg := queryShapePolicyConfig()
	profile := cfg.Profiles["analytics"]
	...
	legacy, err := snapshot.AuthorizeQueryShapeList("client", "credential", "analytics", &discovery)
	if err != nil {
		t.Fatal(err)
	}
	if legacy.RepresentationAvailable() {
		t.Fatal("time-bucket profile unexpectedly has a legacy representation")
	}
	if _, err := legacy.ResponsePayload(); err == nil {
		t.Fatal("unavailable legacy representation returned a payload")
	}
	v2, err := snapshot.AuthorizeQueryShapeList(
		"client", "credential", "analytics", &discovery, domain.QueryShapesMediaTypeV2,
	)
	if err != nil {
		t.Fatal(err)
	}
	payload, err := v2.ResponsePayload()
	if err != nil || !strings.Contains(string(payload), `"kind":"time_bucket"`) ||
		strings.Contains(string(payload), "allow_temporary_table") {
		t.Fatalf("invalid v2 time-bucket disclosure: error=%v payload=%s", err, payload)
	}
}
...
func TestUnavailableLegacyDiscoveryAuditsTheCompleteV3ShapeSet(t *testing.T) {
	cfg := queryShapePolicyConfig()
	profile := cfg.Profiles["analytics"]
	...
	v3, err := snapshot.AuthorizeQueryShapeList(
		"client", "credential", "analytics", &discovery, domain.QueryShapesMediaTypeV3,
	)
	if err != nil {
		t.Fatal(err)
	}
	if v2.RepresentationAvailable() {
		t.Fatal("v2 unexpectedly exposed a profile containing a keyset shape")
	}
	if v2.ShapeCount() != v3.ShapeCount() || v2.ShapeSetHash() == "" || v2.ShapeSetHash() != v3.ShapeSetHash() {
		t.Fatalf("unavailable representation audit identity differs from v3: v2=%d/%q v3=%d/%q",
			v2.ShapeCount(), v2.ShapeSetHash(), v3.ShapeCount(), v3.ShapeSetHash())
	}
}
...
func TestQueryShapeHashesBindTimeBucketAndKeysetPublicMembers(t *testing.T) {
	...
	hashA, err := publicShapeSetHash([]publicAggregateQueryShape{timeBucketA})
	if err != nil {
		t.Fatal(err)
	}
	hashB, err := publicShapeSetHash([]publicAggregateQueryShape{timeBucketB})
	if err != nil {
		t.Fatal(err)
	}
	if hashA == hashB {
	...
	keysetA := publicQueryShapeV3{keyset: &publicKeysetQueryShape{
	...
	keysetB := publicQueryShapeV3{keyset: &keysetBValue}
	keysetHashA, err := publicQueryShapeSetHashV3([]publicQueryShapeV3{keysetA})
	if err != nil {
		t.Fatal(err)
	}
	keysetHashB, err := publicQueryShapeSetHashV3([]publicQueryShapeV3{keysetB})
	if err != nil {
		t.Fatal(err)
	}
	if keysetHashA == keysetHashB {
		t.Fatal("v3 shape hash omitted the keyset order direction")
	}
}
