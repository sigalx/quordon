...
const queryShapeHashDomain = "quordon/query-shapes/v1\x00"
...
func (a AuthorizedQueryShapeList) ShapeSetHash() string {
	...
	if a.mediaType == domain.QueryShapesMediaTypeV3 || !a.RepresentationAvailable() {
		return a.discovery.shapeSetHashV3
	}
...
func (a AuthorizedQueryShapeList) ShapeCount() int {
	...
	if a.mediaType == domain.QueryShapesMediaTypeV3 || !a.RepresentationAvailable() {
		return a.discovery.shapeCountV3
	}
...
func (a AuthorizedQueryShapeList) MediaType() string { return a.mediaType }
func (a AuthorizedQueryShapeList) RepresentationAvailable() bool {
	return len(a.responsePayload()) != 0
}
...
func (a AuthorizedQueryShapeList) responsePayload() []byte {
	...
	if a.mediaType == domain.QueryShapesMediaTypeV2 {
		return a.discovery.payloadV2
	}
	if a.mediaType == domain.QueryShapesMediaTypeV3 {
		return a.discovery.payloadV3
	}
	if a.mediaType == domain.QueryShapesMediaTypeV1 {
		return a.discovery.payloadV1
	}
	return nil
}
...
func (s *Snapshot) ValidateQueryShapeDiscoveryStatic(profileName, adapter string) error {
	...
	if len(publicShapes) != 0 {
		if _, _, err := encodeQueryShapeResponse(
			profileName, s.version, profile.Datasource, adapter, publicShapes, maximum,
		); err != nil {
			return err
		}
	}
	...
	_, _, err = encodeQueryShapeResponseV3(
...
func (s *Snapshot) BuildQueryShapeDiscovery(
	...
	var payloadV2 []byte
	var shapeSetHash string
	if len(publicShapes) != 0 {
		payloadV2, shapeSetHash, err = encodeQueryShapeResponse(
			profileName, s.version, profile.Datasource, adapter, publicShapes, limits.MaxResultBytes,
		)
		if err != nil {
			return QueryShapeDiscovery{}, err
		}
	}
	payloadV3, shapeSetHashV3, err := encodeQueryShapeResponseV3(
		profileName, s.version, profile.Datasource, adapter,
		publicShapes, publicKeysetShapes, limits.MaxResultBytes,
	)
	if err != nil {
		return QueryShapeDiscovery{}, err
	}
	...
	discovery := QueryShapeDiscovery{
		...
		shapeSetHashV3: shapeSetHashV3, semantics: semantics, limits: limits,
		payloadV2: payloadV2, payloadV3: payloadV3,
		resources: resources, fields: fields, shapeCount: len(publicShapes),
		shapeCountV3: len(publicShapes) + len(publicKeysetShapes),
	}
	if len(publicKeysetShapes) == 0 && !aggregateShapesContainTimeBucket(configuredShapes) {
		discovery.payloadV1 = slices.Clone(payloadV2)
	}
	if len(publicKeysetShapes) != 0 {
		discovery.payloadV2 = nil
	}
...
func encodeQueryShapeResponse(
	...
	publicShapes []publicAggregateQueryShape,
	maximum int,
) ([]byte, string, error) {
	...
	if !ok {
		return nil, "", fmt.Errorf("profile %q query-shape response exceeds max_result_bytes", profileName)
	}
	...
	shapeSetHash, err := publicShapeSetHash(publicShapes)
	if err != nil {
		return nil, "", err
	}
...
func (s *Snapshot) AuthorizeQueryShapeList(
	...
	mediaType := domain.QueryShapesMediaTypeV1
	if len(mediaTypes) == 1 {
		mediaType = mediaTypes[0]
	} else if len(mediaTypes) != 0 {
		return AuthorizedQueryShapeList{}, errors.New("query-shape authorization accepts one media type")
	}
	if mediaType != domain.QueryShapesMediaTypeV1 && mediaType != domain.QueryShapesMediaTypeV2 &&
		mediaType != domain.QueryShapesMediaTypeV3 {
		return AuthorizedQueryShapeList{}, errors.New("query-shape authorization requires a supported media type")
	}
	if discovery == nil || discovery.owner != s || discovery.profile != profileName || discovery.policyVersion != s.version ||
		...
		discovery.shapeCountV3 == 0 || len(discovery.payloadV3) == 0 {
	...
	return AuthorizedQueryShapeList{
		...
		operation: domain.OperationListQueryShapes, discovery: discovery, mediaType: mediaType,
	}, nil
}
...
type queryShapeListResponse struct {
	...
	Shapes        []publicAggregateQueryShape `json:"shapes"`
}
...
func (f publicAggregateFilter) MarshalJSON() ([]byte, error) {
	if f.Kind == "predicate" {
		...
		}{Kind: f.Kind, Field: f.Field, Operator: f.Operator, ValueTypes: f.ValueTypes})
	}
	if f.Kind == "group" {
		return json.Marshal(struct {
			Kind        string                  `json:"kind"`
...
func buildPublicAggregateFilter(
	...
	result := publicAggregateFilter{
		Kind: filter.Kind, Field: filter.Field, Operator: filter.Operator,
	}
	if filter.Kind == "predicate" {
...
func queryShapeListResponseSize(response queryShapeListResponse, maximum int) (int, bool) {
	...
	for index, shape := range response.Shapes {
		...
		queryShapeEncodedSize(size, shape)
	}
	size.add(len(`]}`))
...
func publicShapeSetHash(shapes []publicAggregateQueryShape) (string, error) {
	digest := sha256.New()
	_, _ = io.WriteString(digest, queryShapeHashDomain)
	if err := writeJCSShapes(digest, shapes); err != nil {
		return "", fmt.Errorf("canonicalize public query shapes: %w", err)
	}
	return hex.EncodeToString(digest.Sum(nil)), nil
}

func writeJCSShapes(writer hash.Hash, shapes []publicAggregateQueryShape) error {
	_, _ = io.WriteString(writer, "[")
	for index, shape := range shapes {
		if index != 0 {
			_, _ = io.WriteString(writer, ",")
		}
		if err := writeJCSShape(writer, shape); err != nil {
			return err
		}
	}
	_, _ = io.WriteString(writer, "]")
