...
func configuredKeysetSignature(
	...
	for index, order := range shape.OrderBy {
		result.OrderBy[index] = keysetOrderSignature{
			Field: canonicalPolicyIdentifier(order.Field, semantics.CaseInsensitiveFields), Direction: order.Direction,
		}
	}
	return result, true
}
...
func keysetShapeFilterFromConfig(filter *config.AggregateShapeFilter) *queryspec.Filter {
	...
	result := &queryspec.Filter{Kind: filter.Kind, Field: filter.Field, Operator: filter.Operator}
