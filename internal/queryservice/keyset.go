...
// Before datasource semantics are available, use the same conservative ASCII
// case-insensitive normalization as the policy precheck. This keeps denial
// hashes stable across semantics an adapter may later report and across
// commutative filter order without claiming that any resource is authorized.
...
func validateKeysetResponse(result KeysetSelectResult, request queryspec.NormalizedKeysetRequest) error {
	if result.Kind != "keyset" || result.QueryID == "" || result.PolicyProfile == "" || result.PolicyVersion == "" ||
		...
		result.Datasource == "" || result.Adapter == "" ||
		result.Rows == nil ||
	...
	for index, column := range result.Columns {
		if request.Query.Projection[index].Representation == queryspec.RepresentationSourceText && (column.Type != "string" || column.Encoding != "string") {
			return errors.New("invalid source_text result column")
		}
	...
	if result.Page.HasMore {
		...
		for keyIndex, order := range request.Query.OrderBy {
			...
			if !keysetCursorMatchesCell(cursor[keyIndex], result.Columns[projectionIndex], *lastRow[projectionIndex]) {
...
func keysetCursorMatchesCell(value queryspec.KeysetCursorValue, column database.ResultColumn, cell string) bool {
	if !validKeysetResponseCursor(value) {
		return false
	}
	switch value.Type {
	case "integer", "string", "bytes", "date", "datetime":
		return value.Type == column.Type && value.Value == cell
	case "timestamp":
		if column.Type != "datetime" {
			return false
		}
		// Portable timestamp cursors are explicit UTC instants. Existing row
		// cells retain the adapter-neutral datetime text contract, so compare
		// the same fields after removing only the required T/Z markers.
		return value.Value[:10]+" "+value.Value[11:len(value.Value)-1] == cell
	default:
		return false
	}
}
...
func validKeysetResponseCursor(value queryspec.KeysetCursorValue) bool {
	...
	case "date":
		_, ok := queryspec.ParsePortableDate(value.Value)
		return ok
	case "datetime":
		_, _, ok := queryspec.ParsePortableDateTime(value.Value)
		return ok
	case "timestamp":
		_, _, ok := queryspec.ParsePortableTimestamp(value.Value)
		return ok
	default:
		return false
	}
...
func validKeysetResponseCell(value string, column database.ResultColumn) bool {
	...
	case "date":
		_, ok := queryspec.ParsePortableDate(value)
		return ok
	case "datetime":
		_, _, ok := queryspec.ParsePortableDateTime(value)
		return ok
	case "time":
		return queryspec.ValidPortableTime(value)
	case "json":
