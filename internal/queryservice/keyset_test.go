...
func TestKeysetResponseRejectsTemporalValuesOutsidePhysicalRange(t *testing.T) {
	for _, value := range []string{"0000-01-01", "0999-12-31"} {
		if validKeysetResponseCell(value, database.ResultColumn{Type: "date", Encoding: "string"}) {
			t.Fatalf("out-of-range DATE %q passed the service boundary", value)
		}
	}
	for _, value := range []string{"0000-01-01 00:00:00", "0999-12-31 23:59:59"} {
		if validKeysetResponseCell(value, database.ResultColumn{Type: "datetime", Encoding: "string"}) {
			t.Fatalf("out-of-range DATETIME %q passed the service boundary", value)
		}
	}
}
