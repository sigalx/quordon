...
func (s *Service) ListQueryShapes(
	...
	authorized, err := s.policy.AuthorizeQueryShapeList(
		principal, clientIdentifier, profileName, discovery, mediaType,
	)
	s.discoveryMu.RUnlock()
	if err != nil {
	...
	if err := s.audit.Write(ctx, s.queryShapeAllowEvent(
		requestID, principal, clientIdentifier, profileName, authorized, postToken,
	)); err != nil {
		return nil, &Error{Kind: ErrorServiceUnavailable, Err: err}
	}
	if !authorized.RepresentationAvailable() {
		completionData := *postToken
		completionData.resultBytes = queryShapeRepresentationErrorSize(requestID, s.policy.Version())
		if err := s.writeQueryShapeCompletion(
			ctx, requestID, principal, clientIdentifier, profileName, authorized.Datasource(),
			authorized.Adapter(), "error", "representation_not_acceptable", started, &completionData,
		); err != nil {
			return nil, &Error{Kind: ErrorServiceUnavailable, Err: err}
		}
		return nil, &Error{Kind: ErrorRepresentation}
	}
...
func queryShapeRepresentationErrorSize(requestID, policyVersion string) int {
	payload, err := json.Marshal(struct {
		...
		Code: "REPRESENTATION_NOT_ACCEPTABLE", Message: "No acceptable query-shape representation is available",
		RequestID: requestID, PolicyVersion: policyVersion,
	})
	if err != nil {
		panic("marshal fixed query-shape representation error: " + err.Error())
	}
	return len(payload)
}
...
func (s *Service) validateQueryShapeAuditBound(
	...
	for _, identity := range identities {
		for _, mediaType := range []string{
			domain.QueryShapesMediaTypeV1, domain.QueryShapesMediaTypeV2, domain.QueryShapesMediaTypeV3,
		} {
			authorized, err := s.policy.AuthorizeQueryShapeList(
				identity.principal, identity.clientIdentifier, profileName, discovery, mediaType,
			)
			if err != nil {
			...
			completionData := *data
			if !authorized.RepresentationAvailable() {
				errorKind = "representation_not_acceptable"
				completionData.resultBytes = queryShapeRepresentationErrorSize(
					strings.Repeat("r", 64), s.policy.Version(),
				)
			}
			events := []audit.Event{
...
func (s *Service) writeQueryShapeDenial(
	...
	return s.audit.Write(context.WithoutCancel(ctx), s.queryShapeDenialEvent(
...
func (s *Service) writeQueryShapeCompletion(
	...
	return s.audit.Write(context.WithoutCancel(ctx), event)
}
