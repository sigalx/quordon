...
const (
	...
	ErrorRepresentation     ErrorKind = "representation_not_acceptable"
)
...
func (s *Service) Explain(
	...
	if !slices.Contains(s.databases.Capabilities(profile.Datasource), domain.OperationExplainSelect) {
		if err := s.audit.Write(context.WithoutCancel(ctx), audit.Event{
	...
	if err := s.audit.Write(ctx, decision); err != nil {
		return ExplainResult{}, &Error{Kind: ErrorServiceUnavailable, Err: err}
	}
	...
	if err := s.audit.Write(context.WithoutCancel(ctx), completion); err != nil {
		return ExplainResult{}, &Error{Kind: ErrorServiceUnavailable, Err: err}
	}
	if databaseErr != nil {
...
func (s *Service) writeDenial(
	...
	return s.audit.Write(context.WithoutCancel(ctx), audit.Event{
...
func classifyDatabaseAuditError(err error) ErrorKind {
	if database.IsKind(err, database.ErrorRequestTooLarge) {
		return ErrorInvalid
	}
