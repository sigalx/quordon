...
func (a *statisticsTestAdapter) DescribeObjectStatistics(
	...
	a.statisticsCalls++
	if a.err != nil {
		return database.ObjectStatisticsResult{}, a.err
	}
	if a.result != nil {
		return *a.result, nil
	}
...
func TestObjectStatisticsSeparatesUnsupportedViewFromMissingResource(t *testing.T) {
	for _, fixture := range []struct {
		name         string
		databaseKind database.ErrorKind
		serviceKind  ErrorKind
	}{
		{"unsupported view", database.ErrorInvalid, ErrorInvalid},
		{"missing or unsupported physical resource", database.ErrorNotFound, ErrorNotFound},
	} {
