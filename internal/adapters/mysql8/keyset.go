...
ORDER BY ordinal_position`
	...
	for index, projection := range request.Query.Projection {
		...
		column.representation = projection.Representation
		if projection.Representation == queryspec.RepresentationSourceText {
			if err := validateSourceTextType(column.dataType); err != nil {
				return keysetSource{}, err
			}
		}
	...
	for _, order := range request.Query.OrderBy {
		...
		column.representation = order.Representation
...
func validateKeysetColumnType(column keysetSourceColumn) (string, error) {
	if column.representation == queryspec.RepresentationSourceText {
		if err := validateSourceTextType(column.dataType); err != nil {
			return "", err
		}
		...
		_, err := mysqlKeysetTemporalPrecision(column)
		return "string", err
	}
...
func validateKeysetFilterTypes(filter *queryspec.Filter, columns map[string]keysetSourceColumn) error {
	...
	if filter.Representation == queryspec.RepresentationSourceText {
		return validateSourceTextType(column.dataType)
	}
...
func compileKeyset(
	...
	for index, selection := range request.Query.Projection {
		expression := representedField(qualifyAggregateField(selection.Field), selection.Representation, false)
		if selection.Alias != "" {
	...
	for index, column := range source.keyColumns {
		if source.keyKinds[index] != "string" || column.representation == queryspec.RepresentationSourceText {
			continue
		}
		if !validKeysetCharacterMetadata(column) {
	...
	for index, order := range request.Query.OrderBy {
		terms[index] = representedField(qualifyAggregateField(order.Field), order.Representation, true) + " " + strings.ToUpper(order.Direction)
	}
...
var (
	mysqlDateMinimum      = time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC)
	mysqlDateMaximum      = time.Date(9999, time.December, 31, 0, 0, 0, 0, time.UTC)
	mysqlDatetimeMinimum  = time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC)
...
func compileKeysetCursor(
	...
	for index, order := range request.Query.OrderBy {
		...
		for prefix := 0; prefix < index; prefix++ {
			...
			terms[prefix] = representedField(qualifyAggregateField(request.Query.OrderBy[prefix].Field), request.Query.OrderBy[prefix].Representation, true) + " = " + placeholder
		...
		terms[index] = representedField(qualifyAggregateField(order.Field), order.Representation, true) + " " + operator + " " + placeholder
...
func bindKeysetCursor(value queryspec.KeysetCursorValue, column keysetSourceColumn) (any, error) {
	if column.representation == queryspec.RepresentationSourceText {
		if value.Type != "string" || !utf8.ValidString(value.Value) || utf8.RuneCountInString(value.Value) > queryspec.ProtocolMaxCursorStringRunes {
			return nil, &database.Error{Kind: database.ErrorInvalid}
		}
...
func keysetContinuationRequestFits(
	...
	for index, projection := range request.Query.Projection {
		...
		if projection.Representation != "" && !add(len(`,"representation":"source_text"`)) {
			return false
		}
	...
	for index, order := range request.Query.OrderBy {
		if order.Representation != "" && !add(len(`,"representation":"source_text"`)) {
			return false
		}
		if index != 0 && !add(1) {
			return false
...
func addKeysetFilterRequestBound(filter *queryspec.Filter, maximum int, size *int) bool {
	...
	if filter.Representation != "" {
		var ok bool
		*size, ok = addEncodedSize(*size, len(`,"representation":"source_text"`), maximum)
		if !ok {
			return false
		}
	}
