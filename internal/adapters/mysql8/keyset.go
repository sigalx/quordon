...
func (*Adapter) SelectKeyset(
	...
	source, err := validateKeysetSource(
		ctx, conn, request, query.RequiredIndex(), info.lowerCaseTableNames,
	)
	if err != nil {
		return database.KeysetSelectResult{}, err
	}
	...
	statement, args, err := compileKeyset(request, source, query.RequiredIndex(), query.Limits().DeadlineMS)
	if err != nil {
	...
	if err := validateAggregatePlan(plan, query.RequiredIndex(), query.MaximumRowsExaminedPerScan()); err != nil {
		return database.KeysetSelectResult{}, err
	}
	rows, err := conn.QueryContext(ctx, statement, args...)
	if err != nil {
		return database.KeysetSelectResult{}, classifyQueryError(ctx, err)
	}
	...
	if stoppedEarly {
		...
		// restores all session state, and releases the instance DDL guard without
		// synchronously draining the intentionally unread LIMIT result.
	...
	baseTable, err := isBaseTable(ctx, conn, request.Query.Source, info.lowerCaseTableNames)
	if err != nil {
		return database.KeysetSelectResult{}, err
	}
	if !baseTable {
		return database.KeysetSelectResult{}, invalidSourceError()
	}
...
func validateKeysetSource(
	...
	).Scan(&actualSchema, &actualObject, &engine); err != nil {
	...
	if !strings.EqualFold(engine, "InnoDB") {
		return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("keyset source must be InnoDB")}
	}
...
ORDER BY ordinal_position`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&column.datetimePrecision, &column.extra,
		); err != nil {
	...
	indexStatement := keysetIndexMetadataStatement(caseInsensitiveObjects)
	indexRows, err := conn.QueryContext(ctx, indexStatement, actualSchema, actualObject, requiredIndex)
	if err != nil {
		return keysetSource{}, classifyQueryError(ctx, err)
	}
	defer func() { _ = drainAndCloseRows(indexRows) }()
	keyColumns := make([]keysetSourceColumn, 0, len(request.Query.OrderBy))
	keyKinds := make([]string, 0, len(request.Query.OrderBy))
	indexDirections := make([]string, 0, len(request.Query.OrderBy))
	for indexRows.Next() {
		var rowSchema, rowObject, rowIndex, visible string
		var nonUnique bool
		var sequence int
		var prefix sql.NullInt64
		var columnName, direction sql.NullString
		if err := indexRows.Scan(
			&rowSchema, &rowObject, &rowIndex, &nonUnique, &sequence,
			&columnName, &prefix, &direction, &visible,
		); err != nil {
			return keysetSource{}, classifyExecutionError(ctx, err)
		}
		if !tokenIdentifierMatches(rowSchema, request.Query.Source.Schema, caseInsensitiveObjects) ||
			!tokenIdentifierMatches(rowObject, request.Query.Source.Name, caseInsensitiveObjects) ||
			!tokenIdentifierMatches(rowIndex, requiredIndex, true) || nonUnique ||
			sequence != len(keyColumns)+1 || !columnName.Valid || prefix.Valid ||
			!direction.Valid || visible != "YES" {
			return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("required keyset index is unsupported")}
		}
		if len(keyColumns) >= len(request.Query.OrderBy) ||
			!strings.EqualFold(columnName.String, request.Query.OrderBy[len(keyColumns)].Field) {
			return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("required index does not match keyset order")}
		}
		column, ok := columns[strings.ToLower(columnName.String)]
		if !ok || column.nullable || isGeneratedKeysetColumn(column.extra) {
			return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("keyset columns must be non-null physical columns")}
		}
		kind, err := validateKeysetColumnType(column)
		if err != nil {
			return keysetSource{}, err
		}
		if direction.String != "A" && direction.String != "D" {
			return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("required index has unknown direction")}
		}
		keyColumns = append(keyColumns, column)
		keyKinds = append(keyKinds, kind)
		indexDirections = append(indexDirections, direction.String)
	}
	if err := indexRows.Err(); err != nil {
		return keysetSource{}, classifyExecutionError(ctx, err)
	}
	if len(keyColumns) != len(request.Query.OrderBy) || !keysetIndexDirectionCompatible(indexDirections, request.Query.OrderBy) {
		return keysetSource{}, &database.Error{Kind: database.ErrorInvalid, Err: errors.New("required index does not define the complete keyset order")}
	}
...
func keysetIndexMetadataStatement(caseInsensitiveObjects bool) string {
	// EXPRESSION was added to INFORMATION_SCHEMA.STATISTICS in MySQL 8.0.13.
	// COLUMN_NAME is NULL for functional key parts, so the existing fail-closed
	// validation rejects them without selecting that newer metadata column and
	// keeps the adapter compatible with the documented MySQL >= 8.0 range.
	return `SELECT table_schema, table_name, index_name, non_unique, seq_in_index,
       column_name, sub_part, collation, is_visible
FROM information_schema.statistics
WHERE ` + metadataIdentifierComparison("table_schema", "?", caseInsensitiveObjects) + `
  AND ` + metadataIdentifierComparison("table_name", "?", caseInsensitiveObjects) + `
  AND ` + metadataIdentifierComparison("index_name", "?", true) + `
...
func validateKeysetColumnType(column keysetSourceColumn) (string, error) {
	...
	case "TINYINT", "SMALLINT", "MEDIUMINT", "INT", "INTEGER", "BIGINT":
		return "integer", nil
	case "CHAR", "VARCHAR":
		if !strings.EqualFold(column.characterSet, "utf8mb4") || !column.characterMaximumLength.Valid ||
...
func isGeneratedKeysetColumn(extra string) bool {
...
func keysetIndexDirectionCompatible(indexDirections []string, order []queryspec.Sort) bool {
	exact := true
	inverse := true
	for index, direction := range indexDirections {
		wanted := "A"
		if order[index].Direction == "desc" {
			wanted = "D"
		}
		if direction != wanted {
			exact = false
...
func keysetFilterTypeCompatible(valueType, operator string, column keysetSourceColumn) bool {
	...
	case "decimal":
		...
		return err == nil
	case "string":
		// The public string type accepts arbitrary Unicode. Restrict it to
		// utf8mb4 sources so MySQL never has to perform a lossy character-set
		// conversion for a policy-authorized bind.
		if !strings.EqualFold(column.characterSet, "utf8mb4") {
			return false
		}
	...
	case "uuid":
		...
		return slices.Contains([]string{"CHAR", "VARCHAR", "TINYTEXT", "TEXT", "MEDIUMTEXT", "LONGTEXT"}, dataType) &&
...
func compileKeysetCursor(
	...
	for index, order := range request.Query.OrderBy {
		...
		for prefix := 0; prefix < index; prefix++ {
			terms[prefix] = qualifyAggregateField(request.Query.OrderBy[prefix].Field) + " = ?"
		...
		terms[index] = qualifyAggregateField(order.Field) + " " + operator + " ?"
...
func bindKeysetCursor(value queryspec.KeysetCursorValue, column keysetSourceColumn) (any, error) {
	...
	case "string":
		if !utf8.ValidString(value.Value) || utf8.RuneCountInString(value.Value) > int(column.characterMaximumLength.Int64) {
...
func collectKeysetRows(
	...
	for rows.Next() {
		...
		if err := rows.Scan(destinations...); err != nil {
			return false, classifyExecutionError(ctx, err)
		}
		if err := validateKeysetRow(rawValues, columns, source); err != nil {
			return false, err
		}
		separator := 0
		...
		rowSize, finalFits := encodedRowSize(rawValues, columns, remaining)
		if !finalFits {
			moreObserved = true
			break
		}
		...
		cursorSize, cursorFits := encodedKeysetCursorSize(rawValues, source, maximum-moreBase-rowsSize)
		moreSize := moreBase + rowsSize
		if cursorFits {
			moreSize += cursorSize
			cursorFits = moreSize <= maximum
		}
		row := materializeAggregateRow(rawValues, columns)
		result.Rows = append(result.Rows, row)
		provisionalPreviousSize = finalSize
		finalSize = nextFinalSize
		if cursorFits {
			lastSafeMoreSize = moreSize
			lastSafeCursor = materializeKeysetCursor(rawValues, source)
		} else {
			provisional = true
		}
	}
...
func validKeysetCell(value []byte, portableType string) bool {
	...
	case "date":
		return database.ValidDateCell(string(value))
	case "datetime":
		return database.ValidDateTimeCell(string(value))
	case "time":
		return database.ValidTimeCell(string(value))
	case "json":
...
func validKeysetSourceTemporalCell(value []byte, column keysetSourceColumn) bool {
	...
	case "DATETIME":
		return database.ValidDateTimeCell(string(value))
	case "TIMESTAMP":
		return database.ValidTimestampCell(string(value))
	default:
		return true
	}
}
...
func encodedKeysetCursorSize(raw []sql.RawBytes, source keysetSource, maximum int) (int, bool) {
	...
	for index, projectionIndex := range source.keyIndexes {
		...
		default:
			var ok bool
			valueSize, ok = encodedJSONStringSize(value, maximum-size)
			if !ok {
				return 0, false
			}
		}
		itemSize := len(`{"type":,"value":}`) + encodedJSONStringLiteralSize(source.keyKinds[index]) + valueSize
		if index != 0 {
			itemSize++
		}
		var ok bool
...
func materializeKeysetCursor(raw []sql.RawBytes, source keysetSource) []queryspec.KeysetCursorValue {
	result := make([]queryspec.KeysetCursorValue, len(source.keyIndexes))
	for index, projectionIndex := range source.keyIndexes {
		value := string(raw[projectionIndex])
		if source.keyKinds[index] == "bytes" {
			value = base64.StdEncoding.EncodeToString(raw[projectionIndex])
		}
		result[index] = queryspec.KeysetCursorValue{Type: source.keyKinds[index], Value: value}
	}
	return result
}
