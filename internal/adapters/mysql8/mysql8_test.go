...
func TestClassifyExecutionError(t *testing.T) {
	tests := []struct {
		...
		{name: "administrative privilege missing", ctx: context.Background(), err: &mysql.MySQLError{Number: 1227}, kind: database.ErrorUnavailable},
		{name: "view explain privilege missing", ctx: context.Background(), err: &mysql.MySQLError{Number: 1345}, kind: database.ErrorUnavailable},
...
func TestClassifyExplainErrorTreatsInvalidQueryObjectsAsUnsupported(t *testing.T) {
	for _, number := range []uint16{1054, 1146, 1356} {
