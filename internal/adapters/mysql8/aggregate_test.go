...
func TestCompileAggregateUsesPolicyIndexAndBoundValues(t *testing.T) {
	limit := 10
	...
	statement, args, err := compileAggregate(spec, "idx_orders_status", 1500)
	if err != nil {
		t.Fatal(err)
	}
	...
	want := "SELECT /*+ MAX_EXECUTION_TIME(1500) */ `__quordon_source`.`status` AS `status`, COUNT(DISTINCT `__quordon_source`.`customer_id`) AS `customers` FROM `app`.`orders` AS `__quordon_source` FORCE INDEX (`idx_orders_status`) WHERE `__quordon_source`.`status` = ? GROUP BY `__quordon_source`.`status` ORDER BY `customers` DESC LIMIT ?"
	if statement != want {
...
func TestCompileAggregateUsesOneServerOwnedTimeBucketExpression(t *testing.T) {
	...
	statement, args, err := compileAggregate(spec, "idx_created_at", 1500)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompileAggregateKeepsHiddenTimeBucketAliasOutsidePortableNamespace(t *testing.T) {
	...
	statement, _, err := compileAggregate(spec, "idx_created_at", 1500)
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidateAggregatePlanRequiresOneBoundIndexedSource(t *testing.T) {
	...
	for name, plan := range map[string]json.RawMessage{
		...
		"second table":       json.RawMessage(`{"nested_loop":[{"table":{"table_name":"__quordon_source","access_type":"ref","key":"idx_orders_status","rows_examined_per_scan":1}},{"table":{"table_name":"other","access_type":"eq_ref","key":"PRIMARY","rows_examined_per_scan":1}}]}`),
	} {
