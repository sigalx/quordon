...
const (
	...
	ddlGuardCleanupTimeout    = time.Second
	instanceDDLGuardLockSQL   = "LOCK INSTANCE FOR BACKUP"
	instanceDDLGuardUnlockSQL = "UNLOCK INSTANCE"
)
...
func (*Adapter) Validate(ctx context.Context, db *sql.DB) error {
...
// acquireInstanceDDLGuard prevents a trusted concurrent migration from
// replacing the authorized base-table name with a view between type validation
// and the statement that resolves the object. The guard itself names no user
// resource, so an existing view is still rejected before MySQL resolves any of
// its dependencies.
func acquireInstanceDDLGuard(ctx context.Context, conn *sql.Conn) (func() error, error) {
	if _, err := conn.ExecContext(ctx, instanceDDLGuardLockSQL); err != nil {
		// The request may have reached MySQL even if the acknowledgement did not.
		// Never return a possibly guard-owning connection to the pool.
	...
	return func() error {
		...
		if _, err := conn.ExecContext(cleanupContext, instanceDDLGuardUnlockSQL); err != nil {
			discardSQLConnection(conn)
			return classifyExecutionError(cleanupContext, err)
		}
		if err := setConnectionDeadline(conn, time.Time{}); err != nil {
			discardSQLConnection(conn)
			return classifyExecutionError(cleanupContext, err)
		}
		return nil
...
func (*Adapter) Explain(ctx context.Context, db *sql.DB, query policy.AuthorizedQuery) (database.ExplainResult, error) {
	...
	// Pin object-kind validation and EXPLAIN to one pool connection so the
	// local pool cannot validate a table on one connection and execute a view
	// on another.
	conn, err := db.Conn(ctx)
	if err != nil {
	...
	releaseDDLGuard, err := pinAndValidateBaseTable(
		func() (func() error, error) { return acquireInstanceDDLGuard(ctx, conn) },
		func() (bool, error) {
			return isBaseTable(ctx, conn, query.Query().Source, info.lowerCaseTableNames)
		},
	)
	if err != nil {
		return database.ExplainResult{}, err
	...
	// The instance DDL guard already pins the definition. Keep the post-action
	// lookup as a fail-closed invariant check before returning the plan.
	baseTable, err := isBaseTable(ctx, tx, query.Query().Source, info.lowerCaseTableNames)
	if err != nil {
		return database.ExplainResult{}, err
	}
	if !baseTable {
		return database.ExplainResult{}, invalidSourceError()
	}
...
func invalidSourceError() *database.Error {
	return &database.Error{
		...
		Err:  errors.New("query source is not a base table"),
	}
}
...
func classifyQueryError(ctx context.Context, err error) *database.Error {
	...
	if errors.As(err, &mysqlError) &&
		(mysqlError.Number == 1054 || mysqlError.Number == 1146 || mysqlError.Number == 1345 || mysqlError.Number == 1356) {
...
func isBaseTable(
	ctx context.Context,
	queryer queryRower,
	source queryspec.ResourceRef,
	lowerCaseTableNames int,
) (bool, error) {
	statement := baseTableLookupStatement(lowerCaseTableNames)
	var marker int
	if err := queryer.QueryRowContext(ctx, statement, source.Schema, source.Name).Scan(&marker); err != nil {
...
func baseTableLookupStatement(lowerCaseTableNames int) string {
...
FROM information_schema.tables
WHERE ` + comparison + ` AND table_type = 'BASE TABLE'
ORDER BY table_name`
...
JOIN information_schema.columns c
  ON ` + columnJoin + `
WHERE ` + schemaComparison + ` AND ` + objectComparison + ` AND t.table_type = 'BASE TABLE'
...
func (*Adapter) Select(
	...
	releaseDDLGuard, err := pinAndValidateBaseTable(
		func() (func() error, error) { return acquireInstanceDDLGuard(ctx, conn) },
		func() (bool, error) {
			return isBaseTable(ctx, conn, spec.Source, info.lowerCaseTableNames)
		},
	)
	if err != nil {
		return database.SelectResult{}, err
	...
	baseTable, err := isBaseTable(ctx, tx, spec.Source, info.lowerCaseTableNames)
	if err != nil {
		return database.SelectResult{}, err
	}
	if !baseTable {
		return database.SelectResult{}, invalidSourceError()
	}
	if err := tx.Commit(); err != nil {
		return database.SelectResult{}, classifyExecutionError(ctx, err)
	}
...
func compile(spec queryspec.NormalizedSpec, deadlineMS int) (string, []any, error) {
	...
	for _, selection := range spec.Projection {
		...
		if selection.Kind == "field" {
			expression = quote(selection.Field)
		} else {
			argument := "*"
			if selection.Field != "" {
	...
	if len(spec.OrderBy) > 0 {
		...
		for index, sort := range spec.OrderBy {
			fields[index] = qualifySourceField(spec.Source, sort.Field) + " " + strings.ToUpper(sort.Direction)
		}
...
func compileSelect(spec queryspec.NormalizedSpec, deadlineMS, rowLimit int) (string, []any, error) {
	projection := make([]string, 0, len(spec.Projection))
	for _, selection := range spec.Projection {
		expression := qualifySourceField(spec.Source, selection.Field)
		if selection.Alias != "" {
			expression += " AS " + quote(selection.Alias)
		}
		projection = append(projection, expression)
