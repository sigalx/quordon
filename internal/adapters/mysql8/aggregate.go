...
func (a *Adapter) Aggregate(
	...
	columnsByName, err := validateAggregateSource(
		ctx, conn, spec, info.lowerCaseTableNames,
	)
	if err != nil {
		return database.AggregateResult{}, err
	}
	...
	if queryspec.AggregateUsesTimeBucket(spec) || queryspec.AggregateUsesSourceText(spec) {
...
func validateAggregateSource(
	...
	statement := `SELECT t.table_schema, t.table_name
FROM information_schema.tables t
WHERE ` + schemaComparison + ` AND ` + objectComparison + ` LIMIT 1`
	var actualSchema, actualObject string
	if err := conn.QueryRowContext(ctx, statement, spec.Source.Schema, spec.Source.Name).Scan(&actualSchema, &actualObject); err != nil {
...
ORDER BY c.ordinal_position`
	...
	for _, field := range aggregateSourceTextFields(spec) {
		if err := validateSourceTextType(result[strings.ToLower(field)].dataType); err != nil {
			return nil, err
		}
	}
	return result, nil
...
func aggregateResultColumns(
	...
	for index, output := range spec.Projection {
		if output.Kind == "dimension" || output.Kind == "time_bucket" || output.Kind == "numeric_bucket" {
			...
			if output.Representation == queryspec.RepresentationSourceText {
				result[index].Type = "string"
				result[index].Encoding = "string"
			}
			continue
		}
...
func restoreAggregateSessionTimezone(ctx context.Context, conn *sql.Conn, original string) error {
	...
	defer cancel()
	if err := setConnectionDeadline(conn, time.Now().Add(sessionCleanupTimeout)); err != nil {
		discardSQLConnection(conn)
...
func validateAggregatePlan(
	...
	if maximumRows == 0 || !json.Valid(plan) {
		return &database.Error{Kind: database.ErrorInvalid, Err: errors.New("aggregate plan or estimate bound is invalid")}
	}
		...
		case map[string]any:
			for key, child := range typed {
				if key == "materialized_from_subquery" {
					if _, ok := child.(map[string]any); !ok || !allowTemporaryTable {
						invalidNode = true
					}
				}
				if key == "sharing_temporary_table_with" {
					// A cloned materialization references its producer instead
					// of repeating materialized_from_subquery and its sources.
					shared, ok := child.(map[string]any)
					selectID, idOK := shared["select_id"].(json.Number)
					id, err := strconv.ParseUint(selectID.String(), 10, 64)
					if !ok || !idOK || err != nil || id == 0 || !allowTemporaryTable {
						invalidNode = true
					}
				}
	...
	usesRequiredIndex := requiredIndex == ""
	for _, table := range tables {
		...
		if !nameOK || name == "" || !accessOK || !validPlanAccessType(accessType) || !rowsOK {
			return &database.Error{Kind: database.ErrorInvalid, Err: errors.New("aggregate plan read node is malformed")}
		}
		...
		// Derived/materialized result indexes are optimizer-created and cannot
		// satisfy an operator's requirement for a physical source index.
		// table_name is an alias: even a physical read may be named <derived2>.
		_, materialized := table["materialized_from_subquery"]
		_, sharedMaterialization := table["sharing_temporary_table_with"]
		keyValue, keyPresent := table["key"]
		if accessType == "index_merge" && !keyPresent {
			return &database.Error{Kind: database.ErrorInvalid, Err: errMalformedIndexMergePlan}
		}
		if keyPresent {
			key, ok := keyValue.(string)
			if !ok || key == "" {
				return &database.Error{Kind: database.ErrorInvalid, Err: errors.New("aggregate plan index is malformed")}
			}
			usesIndex := false
			if accessType == "index_merge" {
				keyLengths, ok := table["key_length"].(string)
				possibleKeys, keysOK := table["possible_keys"].([]any)
				if !ok || !keysOK {
					return &database.Error{Kind: database.ErrorInvalid, Err: errMalformedIndexMergePlan}
				}
				var err error
				usesIndex, err = indexMergeUsesIndex(key, keyLengths, requiredIndex, possibleKeys)
				if err != nil {
					return &database.Error{Kind: database.ErrorInvalid, Err: err}
				}
			} else {
				// Outside index_merge, key is one native identifier, which
				// can itself contain commas or expression punctuation.
				usesIndex = strings.EqualFold(key, requiredIndex)
			}
			if !materialized && !sharedMaterialization && accessType != "ALL" && usesIndex {
				usesRequiredIndex = true
			}
		}
	}
	if !usesRequiredIndex {
		return &database.Error{Kind: database.ErrorInvalid, Err: errors.New("aggregate plan does not use the required index")}
	}
	return nil
}

func validPlanAccessType(value string) bool {
	switch value {
...
func compileAggregateFilter(filter queryspec.Filter) (string, []any, error) {
	if filter.Representation == queryspec.RepresentationSourceText {
		return compileSourceTextFilter(filter, qualifyAggregateField(filter.Field))
	}
