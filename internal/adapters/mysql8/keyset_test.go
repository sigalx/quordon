...
func TestMySQLKeysetTemporalCursorBindingIsSourceAware(t *testing.T) {
	...
	for _, fixture := range []struct {
		...
		{name: "date below MySQL range", cursor: queryspec.KeysetCursorValue{Type: "date", Value: "0999-12-31"}, column: dateColumn},
		{name: "datetime below MySQL range", cursor: queryspec.KeysetCursorValue{Type: "datetime", Value: "0999-12-31 23:59:59.123456"}, column: datetimeColumn},
...
func TestKeysetCellSeparatesPortableAndMySQLTemporalRanges(t *testing.T) {
	...
	if validKeysetSourceTemporalCell(
		...
		t.Fatal("date below the MySQL physical range passed source-aware validation")
	}
	if validKeysetSourceTemporalCell(
