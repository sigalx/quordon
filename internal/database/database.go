...
func NewManager(cfg config.Config, resolver secrets.Resolver, adapters ...Adapter) (*Manager, []error) {
	...
	for name, datasource := range cfg.Datasources {
		...
		if sourceTextProblem != nil {
			manager.sources[name] = source{adapter: adapter, adapterName: datasource.Adapter, capabilities: capabilities, features: features, initErr: sourceTextProblem, required: datasource.RequiredForReadiness}
			problems = append(problems, sourceTextProblem)
			continue
		}
		dsn := datasource.DSN
		if dsn == "" {
