...
func TestUnsupportedViewStatisticsRuntimeMatchesOpenAPI(t *testing.T) {
	contract := loadTableStatisticsContractValidator(t)
	...
	var result apiError
	if err := json.Unmarshal(response.Body.Bytes(), &result); err != nil || result.Code != "UNSUPPORTED_QUERY" {
		t.Fatalf("error = %+v %v", result, err)
	}
	valid, validationErrors := contract.ValidateHttpResponse(request, response.Result())
	if !valid {
		t.Fatalf("runtime view error violates OpenAPI: %v", validationErrors)
	}
	if len(sink.Events) != 2 || sink.Events[1].ErrorKind != "invalid" {
		t.Fatalf("view statistics audit = %+v", sink.Events)
	}
	invalid := response.Result()
	invalid.Body = io.NopCloser(strings.NewReader(strings.Replace(response.Body.String(), "UNSUPPORTED_QUERY", "NOT_FOUND", 1)))
	if valid, _ := contract.ValidateHttpResponse(request, invalid); valid {
		t.Fatal("422 accepted the wrong stable code")
	}
}
