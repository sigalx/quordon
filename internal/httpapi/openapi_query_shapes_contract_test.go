...
func TestQueryShapesNegotiatesTimeBucketRepresentation(t *testing.T) {
	contract := loadQueryShapesContractValidator(t)
	defer contract.Release()
	server, sink, closeDatabases := successfulTimeBucketContractServer(t)
	defer closeDatabases()

	legacyRequest := httptest.NewRequest(http.MethodGet, "/query-shapes?profile=reader", nil)
	legacyRequest.SetBasicAuth("client", "secret")
	legacyResponse := httptest.NewRecorder()
	server.Handler().ServeHTTP(legacyResponse, legacyRequest)
	if legacyResponse.Code != http.StatusNotAcceptable || legacyResponse.Header().Get("Vary") != "Accept" {
		t.Fatalf("legacy response status=%d Vary=%q body=%s", legacyResponse.Code, legacyResponse.Header().Get("Vary"), legacyResponse.Body.String())
	}
	var legacyError apiError
	if err := json.Unmarshal(legacyResponse.Body.Bytes(), &legacyError); err != nil || legacyError.Code != "REPRESENTATION_NOT_ACCEPTABLE" {
		t.Fatalf("legacy error=%+v decode=%v", legacyError, err)
	}
	if len(sink.Events) < 2 || sink.Events[len(sink.Events)-1].ResultBytes != legacyResponse.Body.Len() {
		t.Fatalf("406 completion audit does not contain exact response bytes: events=%+v body=%d", sink.Events, legacyResponse.Body.Len())
	}
	legacyContractRequest := httptest.NewRequest(
		http.MethodGet, "http://quordon.test/query-shapes?profile=reader", nil,
	)
	legacyContractRequest.SetBasicAuth("client", "secret")
	valid, validationErrors := contract.ValidateHttpResponse(legacyContractRequest, legacyResponse.Result())
	if !valid {
		t.Fatalf("legacy 406 runtime response violates OpenAPI: %v; body=%s", validationErrors, legacyResponse.Body.String())
	}

	v2Request := httptest.NewRequest(http.MethodGet, "/query-shapes?profile=reader", nil)
	v2Request.Header.Set("Accept", domain.QueryShapesMediaTypeV2)
	v2Request.SetBasicAuth("client", "secret")
	v2Response := httptest.NewRecorder()
	server.Handler().ServeHTTP(v2Response, v2Request)
	if v2Response.Code != http.StatusOK || v2Response.Header().Get("Content-Type") != domain.QueryShapesMediaTypeV2 ||
		!strings.Contains(v2Response.Body.String(), `"kind":"time_bucket"`) {
		t.Fatalf("v2 response status=%d type=%q body=%s", v2Response.Code, v2Response.Header().Get("Content-Type"), v2Response.Body.String())
	}
	contractRequest := httptest.NewRequest(http.MethodGet, "http://quordon.test/query-shapes?profile=reader", nil)
	contractRequest.Header.Set("Accept", domain.QueryShapesMediaTypeV2)
	contractRequest.SetBasicAuth("client", "secret")
	valid, validationErrors = contract.ValidateHttpResponse(contractRequest, v2Response.Result())
	if !valid {
		t.Fatalf("v2 runtime response violates OpenAPI: %v; body=%s", validationErrors, v2Response.Body.String())
	}
}
