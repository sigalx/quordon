...
func TestKeysetPaginationDesignPromotionGuardsAreExplicit(t *testing.T) {
	...
	for _, invariant := range []string{
		...
		"publish-all-shapes-through-one-json-contract",
...
func TestKeysetPaginationDesignDiscoveryJSON(t *testing.T) {
	...
	valid, validationErrors := contract.ValidateHttpResponse(request, response)
	if !valid {
		t.Fatalf("JSON discovery response violates design contract: %v", validationErrors)
	}
	...
	valid, validationErrors = contract.ValidateHttpResponse(request, response)
	if !valid {
		t.Fatalf("JSON discovery hardcodes a concrete adapter: %v", validationErrors)
	}
	...
	if valid {
		t.Fatal("JSON discovery accepted an impossible NULL predicate arity")
	}
	...
	valid, _ = contract.ValidateHttpResponse(request, response)
	if valid {
		t.Fatal("JSON discovery accepted LIKE with an integer placeholder type")
	}
	...
	valid, _ = contract.ValidateHttpResponse(request, response)
	if valid {
		t.Fatal("JSON discovery accepted heterogeneous IN placeholder types")
	}
	...
	for _, validFilter := range []string{
		...
		valid, validationErrors = contract.ValidateHttpResponse(request, response)
		if !valid {
			t.Fatalf("JSON discovery rejected a valid statically typed filter: %v", validationErrors)
		}
	}
}
...
func TestQueryShapeJSONContract(t *testing.T) {
	contract := loadOpenAPI31Validator(t)
	defer contract.Release()
...
func TestQueryShapeJSONIncludesTimeBuckets(t *testing.T) {
	contract := loadOpenAPI31Validator(t)
	defer contract.Release()
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"kind":"time_bucket"`) {
		t.Fatalf("JSON discovery status=%d body=%s", response.Code, response.Body.String())
	}
	if valid, errs := contract.ValidateHttpResponse(request, response.Result()); !valid {
		t.Fatalf("JSON response violates OpenAPI: %v", errs)
	}
}
...
func TestSuccessfulKeysetDiscoveryHandlerMatchesExecutableOpenAPI31(t *testing.T) {
	...
	for _, mediaType := range []string{"", "*/*", domain.QueryShapesMediaType} {
		...
		response := httptest.NewRecorder()
		server.Handler().ServeHTTP(response, request)
		if response.Code != http.StatusOK || response.Header().Get("Vary") != "Accept" {
