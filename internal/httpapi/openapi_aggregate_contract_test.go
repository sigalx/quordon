...
type aggregateContractSourceWork struct {
	...
	RowsExaminedMetric struct {
		SourceNodeIdentification struct {
			RequiredCount int    `yaml:"required-matching-node-count"`
			RejectOthers  string `yaml:"any-other-table-or-materialized-node"`
		...
		Ambiguous          string `yaml:"multiple-or-ambiguous-source-nodes"`
...
func TestAggregateContractBindsCompleteShapeAndRowsExaminedMetric(t *testing.T) {
	...
	if metric.SourceNodeIdentification.RequiredCount != 1 || metric.SourceNodeIdentification.RejectOthers != "reject" ||
