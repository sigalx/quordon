...
func TestDecodeStrictKeysetTemporalCursorBranches(t *testing.T) {
	after := strings.Replace(
		validKeysetRequestJSON,
		`"page":{"kind":"first"}`,
		`"page":{"kind":"after","cursor":[{"type":"integer","value":"1"}]}`,
	...
	for _, fixture := range []struct {
		...
		{name: "date lower portable bound", cursorType: "date", value: "0001-01-01"},
		{name: "datetime nine digits", cursorType: "datetime", value: "9999-12-31 23:59:59.123456789"},
		{name: "timestamp UTC", cursorType: "timestamp", value: "2026-09-15T12:34:56.123456789Z"},
	} {
		t.Run(fixture.name, func(t *testing.T) {
			body := strings.Replace(
				after,
				`"type":"integer","value":"1"`,
				`"type":"`+fixture.cursorType+`","value":"`+fixture.value+`"`,
				1,
			)
			if _, err := DecodeStrictKeyset([]byte(body), 8, 50, 100); err != nil {
				t.Fatalf("valid temporal cursor was rejected: %v", err)
			}
		})
	}
	...
	invalid := []struct {
		...
		{name: "date null", cursor: `{"type":"date","value":null}`},
		{name: "date as number", cursor: `{"type":"date","value":20260915}`},
		{name: "date zero year", cursor: `{"type":"date","value":"0000-01-01"}`},
		{name: "date invalid calendar", cursor: `{"type":"date","value":"2026-02-29"}`},
		{name: "datetime T separator", cursor: `{"type":"datetime","value":"2026-09-15T12:34:56"}`},
		{name: "datetime timezone", cursor: `{"type":"datetime","value":"2026-09-15 12:34:56Z"}`},
		{name: "timestamp offset", cursor: `{"type":"timestamp","value":"2026-09-15T12:34:56+00:00"}`},
		{name: "timestamp lowercase", cursor: `{"type":"timestamp","value":"2026-09-15t12:34:56z"}`},
		{name: "timestamp infinity", cursor: `{"type":"timestamp","value":"infinity"}`},
		{name: "timestamp overprecision", cursor: `{"type":"timestamp","value":"2026-09-15T12:34:56.1234567890Z"}`},
		{name: "case folded discriminator", cursor: `{"type":"Timestamp","value":"2026-09-15T12:34:56Z"}`},
		{name: "branch extra member", cursor: `{"type":"date","value":"2026-09-15","timezone":"UTC"}`},
	}
	for _, fixture := range invalid {
		t.Run(fixture.name, func(t *testing.T) {
			body := strings.Replace(after, `{"type":"integer","value":"1"}`, fixture.cursor, 1)
			if _, err := DecodeStrictKeyset([]byte(body), 8, 50, 100); err == nil {
				t.Fatal("invalid temporal cursor was accepted")
			}
		})
	}
}
