...
func validateKeysetCursorValue(path string, cursor KeysetCursorValue) error {
	switch cursor.Type {
	case "integer":
		if err := validateUniversalMySQLInteger(cursor.Value); err != nil {
...
func validateUniversalMySQLInteger(value string) error {
	if value == "" || value[0] == '+' || value == "-0" || len(value) > 20 {
		return errors.New("must be a canonical MySQL integer")
	}
	if value[0] == '-' {
		...
		if _, err := strconv.ParseInt(value, 10, 64); err != nil {
			return errors.New("is outside the universal MySQL integer range")
		}
		return nil
	}
