...
func (w selectionWire) decode() (Selection, error) {
	...
	return Selection{
		Kind: w.Kind.Value, Field: w.Field.Value, Function: w.Function.Value, Alias: w.Alias.Value,
	}, nil
}
...
func (w filterWire) decode(depth, maxDepth int) (Filter, error) {
	...
	result := Filter{Kind: w.Kind.Value, Field: w.Field.Value, Operator: w.Operator.Value}
	switch w.Kind.Value {
	case "predicate":
		if !w.Field.Set {
...
func (w sortWire) decode() (Sort, error) {
	...
	return Sort{Field: w.Field.Value, Direction: w.Direction.Value}, nil
}
