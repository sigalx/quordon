...
func canonicalKeysetRequestSignature(shape KeysetSelectShape) [sha256.Size]byte {
	...
	for index, output := range shape.Projection {
		value.Projection[index] = projection{Field: strings.ToLower(output.Field), Alias: strings.ToLower(output.Alias)}
	}
	if shape.Filter != nil {
	...
	for index, item := range shape.OrderBy {
		value.OrderBy[index] = order{Field: strings.ToLower(item.Field), Direction: item.Direction}
	}
