...
allow audit adapter берёт instance DDL guard до первого lookup объекта, требует
физическую InnoDB `BASE TABLE` и читает только allowlisted колонки
...
MVP генерирует только:

```text
EXPLAIN FORMAT=JSON <compiled SELECT>
```

Источник запроса должен быть физической таблицей MySQL (`BASE TABLE`). После
policy authorization adapter сначала получает instance DDL guard, не называющий
клиентский resource, и только затем проверяет тип объекта через системный каталог.
Сравнение schema/object следует `lower_case_table_names`: при значении `0` оно
бинарное, при `1/2` — регистронезависимое. `LOCK INSTANCE FOR BACKUP` удерживается
до окончания read-only transaction, поэтому concurrent DDL не может подменить
проверенную таблицу view. Обычные и case-colliding views отклоняются до того, как
MySQL разрешит их зависимости; неизвестные объекты также не возвращают план.
...
Клиент не выбирает explain mode. `EXPLAIN ANALYZE` запрещён, поскольку выполняет statement. MySQL допускает исполняемые эффекты в отдельных конструкциях даже при `EXPLAIN SELECT`, поэтому структурное ограничение запроса не заменяет scoped `SELECT` grants. Для DDL guard дополнительно требуется динамическое право `BACKUP_ADMIN`; прочие административные права gateway не нужны. MVP не поддерживает views, subqueries и пользовательские функции.

Исполняемый `select` использует только field projections, один physical
`BASE TABLE`, allowlisted filters/sorting, bind parameters и обязательный
server-side `LIMIT`. Вид объекта проверяется после получения instance DDL guard,
который удерживается до commit и не допускает замены объекта между validation и
execution. Aggregates и
`group_by` для исполняемого запроса отклоняются до обращения к DBMS.
...
key, requested limit, policy-owned visible unique index и estimate bound.
Cursor не выбирает SQL или индекс: это закрытый typed tuple точных значений
`integer`/`string`/`bytes`, чья arity и physical source types повторно
проверяются adapter до SELECT.

Для temporal filter adapter связывает `TIMESTAMP` как UTC instant под
проверенной UTC session, но сохраняет исходные wall-clock fields для
timezone-naive `DATETIME`; превышение physical fractional precision даёт
`422 UNSUPPORTED_QUERY` до `EXPLAIN`/`SELECT`. До DBMS также проверяются
физические диапазоны MySQL (`DATE`/`DATETIME` с 1000 года, `TIMESTAMP` от
...
Произвольные typed string filters допускаются только для `utf8mb4` source,
а UUID — только для character source шириной не менее 36 символов, чтобы
MySQL не выполнял неявную lossy conversion или truncation.

Keyset adapter требует physical InnoDB table и полный `NOT NULL` unique index
без prefix, expression или invisible parts. `FORCE INDEX` входит и в
`EXPLAIN FORMAT=JSON`, и в выполняемый SELECT; plan admission требует тот же
index, запрещает `ALL` и ограничивает `rows_examined_per_scan`. Лексикографический
seek predicate генерируется только из quoted server-owned identifiers и bind
parameters. DDL guard, read-only `REPEATABLE READ` transaction, UTC session,
deadline и bounded cleanup действуют на одном physical connection. При раннем
завершении после формирования safe prefix connection закрывается и
discard-ится до `Rows.Close`, поэтому unread result не дренируется синхронно
под capacity/transaction/DDL guard. Отдельные
HTTP pages не имеют общего snapshot, поэтому concurrent DML остаётся явно
принятым свойством keyset pagination.

Исполняемый aggregate использует отдельный contract module
`openapi/aggregate.yaml`, включённый в корневой OpenAPI. Adapter использует один
physical connection и instance DDL guard от первой проверки source до
завершения read-only transaction.
Под guard разрешается только `InnoDB BASE TABLE`; adapter явно устанавливает
`REPEATABLE READ` и открывает read-only consistent snapshot. Другие engines
возвращают `422` до выполнения aggregate, поскольку DDL guard сам по себе не
изолирует concurrent DML.

Runtime допускает только явно разрешённые оператором indexed
aggregate shapes. План проверяется через `EXPLAIN FORMAT=JSON` под тем же guard:
required index, maximum `rows_examined_per_scan` и запреты на temporary table и
filesort принадлежат policy и не управляются клиентом. Эта проверка является
эвристическим admission control, а не жёсткой границей: stale statistics, data
skew и широкий index range могут увеличить фактическую работу. Оператор принимает
этот остаточный риск при включении shape. `LIMIT` результата, plan estimate,
deadline и concurrency не объявляются DBMS-enforced source-row limit.
Required index из совпавшего policy shape компилируется adapter в `FORCE INDEX`
одного и того же SELECT для preflight и execution; клиент не передаёт index name.
...
Rows admission metric вычисляется однозначно: adapter рекурсивно обходит JSON
plan, обычно требует ровно один `table` node с server-owned source alias и не
допускает других table/materialized nodes. Исключение — строгий zero-table
envelope, содержащий только `query_block.select_id: 1` и точное MySQL-сообщение
`Impossible WHERE` либо `no matching row in const table`. Он означает
доказуемо пустой результат или не более одного unique-index probe; source и
visible required index уже проверены под DDL guard, а тот же policy-owned
`FORCE INDEX` присутствует в EXPLAIN и исполняемом SQL. Любой другой no-table
plan fail-closed отклоняется. Для обычного table plan используется только его
`rows_examined_per_scan`; `rows_produced_per_join`, `filtered` и cost не
участвуют. Значение обязано быть неотрицательным JSON integer, разбирается как
base-10 `uint64` до native conversion и сравнивается с policy maximum. Missing,
null, string, fraction, negative, malformed, ambiguous и overflow значения дают
`422 UNSUPPORTED_QUERY` до aggregate execution.
Policy `maximum_rows_examined_per_scan` является обязательным положительным
plain base-10 YAML integer в диапазоне `uint64`; quoted, fractional, exponent,
invalid или overflowing configuration блокирует startup до typed decode.
Required index обязан существовать и быть visible до компиляции `FORCE INDEX`.
Поле `key` единственного source table node обязано совпасть с canonical required
index, а `access_type` не может быть `ALL`.
...
- plan возвращается только для уже авторизованной физической таблицы; views
  отклоняются до `EXPLAIN`, чтобы их раскрытый plan не выдал закрытые зависимости;
