...
- instance DDL guard до первой проверки типа source и до завершения operation;
- case-aware проверка, что source является `BASE TABLE`, под этим guard;
...
- получить instance DDL guard до первого source lookup и удерживать его на одном
  physical connection до завершения read-only transaction;
- принимать только `InnoDB BASE TABLE`, явно устанавливать `REPEATABLE READ` и
  открывать read-only consistent snapshot; другие engines возвращают `422`;
- принимать только operator-curated indexed aggregate shapes с policy-owned
  required index и maximum `rows_examined_per_scan`;
- выполнить `EXPLAIN FORMAT=JSON` под тем же guard и отклонить планы без
  требуемого index access, сверх row estimate, с неразрешёнными temporary table
  или filesort;
- скомпилировать required index из policy shape как quoted `FORCE INDEX` в один
  SELECT, используемый и для preflight, и для execution; клиент не выбирает index;
...
token.
Для plan admission рекурсивно ищется ровно один `table` node с server-owned
source alias; другие table/materialized nodes запрещены. Единственная метрика —
его integer `rows_examined_per_scan`, overflow-safe разобранный как `uint64`.
`rows_produced_per_join`, `filtered` и cost не используются. Missing, malformed,
ambiguous или overflowing metric возвращает `422` до aggregate execution.
Обязательный policy `maximum_rows_examined_per_scan` — positive `uint64`; invalid
или overflowing config блокирует startup. Plan `key` должен совпасть с canonical
required index, а `access_type: ALL` всегда отклоняется.
