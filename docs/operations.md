...
`source` обязан быть физической таблицей (`BASE TABLE`). Adapter проверяет это
через `information_schema.tables` после policy authorization. Перед первой
case-aware проверкой adapter получает `LOCK INSTANCE FOR BACKUP` и удерживает
его до завершения read-only transaction. Поэтому concurrent DDL не может
заменить таблицу view между validation и `EXPLAIN`/`SELECT`, а уже существующий
view отклоняется до разрешения его зависимостей. View, case-colliding view или
неизвестный объект возвращает `422 UNSUPPORTED_QUERY` без плана или результата.

Datasource user должен иметь только нужные `SELECT` grants на curated objects и
динамическое право `BACKUP_ADMIN` на instance. Это право нужно исключительно для
`LOCK INSTANCE FOR BACKUP`; readiness проверяет возможность получить и снять
guard. Guard блокирует DDL на всём instance на время operation, поэтому migration
и Quordon query могут кратковременно ожидать друг друга в пределах deadline.
Ошибка или timeout при снятии guard приводит к закрытию соединения, а не к его
возврату в pool с удерживаемой блокировкой.

Тот же `BACKUP_ADMIN` нужен `describe_object_statistics`: guard удерживается от
первой проверки physical InnoDB table до последнего чтения
`INFORMATION_SCHEMA.PARTITIONS`. Endpoint не выполняет table scan или точный
`COUNT(*)`, но engine estimates могут быть stale и не образуют DML snapshot.
...
Schema discovery читает только `BASE TABLE` metadata и фильтрует objects/columns
...
Capability `aggregate` принимает только operator-curated shapes из policy.
Для каждого shape operator обязан выбрать реально существующий selective index,
разумный `maximum_rows_examined_per_scan`, короткий deadline и небольшой group
limit. Adapter компилирует этот index как `FORCE INDEX` и проверяет MySQL JSON
plan, однако optimizer estimate остаётся эвристикой и может недооценивать
фактическую работу при stale statistics, skew или широком range. Поэтому shapes
следует проверять на production-like statistics и пересматривать после заметных
изменений распределения данных или индексов. Разрешены только InnoDB physical
tables; aggregate удерживает тот же instance DDL guard и требует `BACKUP_ADMIN`.

Capability `select_keyset` также включается только для operator-curated shapes.
Перед эксплуатацией каждого shape проверьте полный видимый unique index,
совместимые `NOT NULL` key columns, worst-case filter range и
`maximum_rows_examined_per_scan`. Adapter закрепляет index через `FORCE INDEX`
и выполняет plan preflight, но estimate остаётся эвристикой. Клиенту следует
копировать `next_cursor` без преобразования типов и не ожидать общего snapshot
между HTTP pages. Cursor values, строки результата, required index и SQL в audit
не записываются; completion содержит только shape name, row count и `has_more`.
...
получает connection и не вызывает adapter. V3-представление discovery
необходимо для `select_keyset`; v1/v2 получают `406`, если profile содержит
keyset shape. Наблюдение другой операцией новых
...
source не является физической таблицей. Отказ MySQL в доступе или
...
1. Local MySQL 8 с production-подобной minor version, scoped `SELECT` grants и
   динамическим `BACKUP_ADMIN` для instance DDL guard.
