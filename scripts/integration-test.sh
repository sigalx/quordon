...
if ! docker compose -f "$compose_file" up --build --detach --wait; then
...
service_grants=$(docker compose -f "$compose_file" exec -T mysql \
  mysql --protocol=tcp --host=127.0.0.1 --user=quordon \
  --password=quordon-password --batch --skip-column-names --execute="SHOW GRANTS")
if printf '%s\n' "$service_grants" | grep --quiet 'BACKUP_ADMIN\|SHOW VIEW'; then
  echo 'Integration service user unexpectedly has BACKUP_ADMIN or SHOW VIEW.' >&2
  exit 1
fi
...
printf '%s\n' "$query_shapes" | grep --quiet '"name":"temporal_events_by_date"'
printf '%s\n' "$query_shapes" | grep --quiet '"name":"temporal_events_by_datetime"'
printf '%s\n' "$query_shapes" | grep --quiet '"name":"temporal_events_by_timestamp"'
...
sh ./scripts/integration-keyset-characters.sh
...
if printf '%s\n' "$audit_output" | grep --quiet 'ddl_guard_mode'; then
  echo 'Audit output contains the removed DDL guard field.' >&2
  exit 1
fi
